/**
 * HKSFC News Extractor (Deno Version)
 *
...
const SELECTORS = {
  ...
  link: 'a.news-link, a[href*="/news/"], a, td:nth-child(2) a',
...
export class HKSFCNewsExtractor extends BaseExtractor<HKSFCRawInput, HKSFCExtractResult> {
   ...
   */
  private extractSingleArticle(element: any, baseUrl: string, index: number): HKSFCNews {
    // Extract title
    ...
    const url = this.resolveUrl(linkElement?.getAttribute('href') || '', baseUrl);
    ...
    // Extract PDF link
    const pdfElement = element.querySelector(SELECTORS.pdfLink);
    const pdfUrl = pdfElement?.getAttribute('href')
      ? this.resolveUrl(pdfElement.getAttribute('href') || '', baseUrl)
      : undefined;
  ...
  /**
   * Categorize article based on title, category tag, and URL
   */
