-- Reset npm_categories if needed (but preserve existing data if any)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM npm_categories LIMIT 1) THEN
    -- Insert the predefined categories only if table is empty
...
-- Create a function to update package counts in categories if it doesn't exist
