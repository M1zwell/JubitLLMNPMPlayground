 ...
 * Database constraint allows: consultation, corporate, enforcement, news, policy
 */
function determineFilingType(title, refNo, feedType) {
  const titleLower = title.toLowerCase();

  // Circulars - policy or corporate
  if (feedType === 'Circulars') {
    if (titleLower.includes('virtual asset') || titleLower.includes('product') || titleLower.includes('service')) {
      return 'policy';
    }
    return 'corporate';
  }
  ...
  if (titleLower.includes('guideline') || titleLower.includes('guidance') || titleLower.includes('circular')) {
    return 'policy';
  }
  if (titleLower.includes('appointment') || titleLower.includes('appoint') || titleLower.includes('ceo')) {
    return 'corporate';
  }

  // Default to news for press releases
  return 'news';
}
